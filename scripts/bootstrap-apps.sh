...
function apply_sops_secrets() {
    ...
    local -r secrets=(
        "${ROOT_DIR}/secrets/sops/github-deploy-key.sops.yaml"
        "${ROOT_DIR}/secrets/sops/cluster-secrets.sops.yaml"
        "${ROOT_DIR}/secrets/sops/sops-age.sops.yaml"
    )
...
function main() {
    check_env KUBECONFIG IONOSCONFIG
